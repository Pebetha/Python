...
//Basic Select
db.Products.find({});//select*from
db.Products.find({}).limit(5);//เงือนไขมากสุดแค่5
db.Products.find({}).count();//ใช้นับ

db.Products.find({}).sort({"pprice":1});//ใช้เรียงข้อมูลโดยใช้ผ่านdatadic โดยมีแค่ 1 คือ น้อยไปมาก กับ -1 มากไปน้อย
db.Products.find({}).sort({"pprice":-1});//ใช้เรียงข้อมูลโดยใช้ผ่านdatadic โดยมีแค่ 1 คือ น้อยไปมาก กับ -1 มากไปน้อย
db.Products.find({}).sort({"pprice":1,"pname":1});//สามารถเรียงได้หลายระดับโดยใช้,
db.Products.find({}).sort({"pbrand":1,"pprice":-1});

// where
db.Products.find({"pprice":{"$gt":10000}}).sort({"pprice":1});//where pprice > 10000 $gt = เครื่องหมายมากกว่า
db.Products.find({"pprice":{"$lte":5000}});//.count()
db.Products.find({"psize":{"$in":[5,6]}}).sort({"pprice":1});//where in

// $and , $or
// SAMSUNG , price < 10000
db.Products.find({"$and":[{"pbrand":"Samsung"},{"pprice":{"$lt":10000}}]}).count();
// pzis < 5 and pprice > 10000
db.Products.find({"$and":[{"psize":{"$lt":5}},{"pprice":{"$lt":10000}}]});

//วิธีการเข้าถึงข้อมูล
db.Products.find({"psize.x":{"$lt":5}});
db.Products.find({"psize.x":{"$exists":true}});//existคือเปรียบเทียบว่ามีมั้ยtrue ถ้าไม่มีก็ใส่ false
db.Products.find({"psize.x":{"$exists":false}});//existคือเปรียบเทียบว่ามีมั้ยtrue ถ้าไม่มีก็ใส่ false

// where like 
// where pname like 'V%'
db.Products.find({"pname":{"$regex":"^V","$options":"i"}});//{"$regex":"^V","$options":"i"}}); ขึ้นต้นด้วย V ignorecase ตัวเลขใหญ่หาเจอหมด

// where pname like '%s'
db.Products.find({"pname":{"$regex":"es$","$options":"i"}});//{"$regex":"es$"}});ลงท้ายด้วยs

// where pname like '%one%'
db.Products.find({"pname":{"$regex":"one"}});